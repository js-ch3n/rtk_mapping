...
Set ``` pcd_save_enable ``` in launchfile to ``` 1 ```. All the scans (in global frame) will be accumulated and saved to the file ``` FAST_LIO/PCD/scans.pcd ```

### 3.6 RTK Mapping Modes

FAST_LIO supports three RTK/GNSS operating modes for globally-registered mapping. All modes use the `gnss_conversion` package to preprocess raw `sensor_msgs/NavSatFix` into zero-based `nav_msgs/Odometry` on `/rtk_odom`.

**Pipeline:**
```
GNSS NavSatFix → gnss_conversion → /rtk_odom → FAST_LIO (rtk_mode)
```
...
|------|:---:|:---:|---|
| 1. Original FAST_LIO | `false` | — | Pure LiDAR-IMU odometry, no RTK |
| 2. RTK Localization | `true` | `false` | RTK directly overrides EKF state (legacy) |
| 3. RTK Fusion | `true` | `true` | RTK added as measurement residual in IEKF |

**Mode 3 parameters:**

| Param | Default | Description |
|-------|---------|-------------|
| `rtk_fuse_en` | `false` | Enable RTK measurement fusion |
| `rtk_repeat_n` | `5` | RTK trust factor: each RTK position row repeated N times. Higher = trust RTK more. Equivalent to `R_rtk = R_lidar / N`. N=1 treats RTK and LiDAR equally. |

**How `rtk_repeat_n` works:**

RTK position measurement (X,Y,Z) is appended N times to the IEKF measurement Jacobian. Repeating the same measurement N times under noise `R_lidar` is mathematically equivalent to using a single measurement with noise `R_lidar/N`. Choose based on your RTK accuracy:

| RTK accuracy | Recommended N |
|-------------|:---:|
| 1 cm | 10 |
| 2 cm | 3–5 |
| 5 cm | 1 (equal to LiDAR) |

**Config example** (`config/velodyne_rtk.yaml`):
```yaml
rtk:
    rtk_mode_en: true
    rtk_topic: "/rtk_odom"
    rtk_fuse_en: true       # Mode 3: fusion
    rtk_repeat_n: 5         # trust RTK ~5x more than LiDAR
```
...
# Terminal 2: FAST_LIO with RTK fusion
...
|---|---|---|
| LiDAR matching | Disabled | Full IEKF |
| RTK role | Replaces state | Constrains state |
| Trajectory smoothness | Follows RTK (may jitter) | Smoothed by LiDAR |
| RTK dropout behavior | Stops | Falls back to LiDAR-only |
| Drift without RTK | Immediate drift | LiDAR-IMU maintains |
